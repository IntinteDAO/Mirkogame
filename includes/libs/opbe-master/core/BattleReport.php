...
class BattleReport
{
    ...
    public function getAttackerDebris()
    {
		...
		$factor = constant('SHIP_DEBRIS_FACTOR');
        $metal *= $factor;
        $crystal *= $factor;
    ...
    public function getDefenderDebris()
    {
		...
		$defenseDerbisMetall = 0;
        $defenseDerbisCrystal = 0;
        ...
        foreach ($this->getDefendersLostUnits(!REPAIRED_DO_DEBRIS) as $idPlayer => $player)
        {
            foreach ($player as $idFleet => $fleet)
            {
                foreach($fleet as $role => $values)
                {
                    ...
                    {
						if($idShipType > 400){
							$defenseDerbisMetall += $lost[0];
							$defenseDerbisCrystal += $lost[1];   
						}
						else{
							$metal += $lost[0];
							$crystal += $lost[1];      
		...
		$shipfactor = constant('SHIP_DEBRIS_FACTOR');
					
		$defensefactor = constant('DEFENSE_DEBRIS_FACTOR');
					
		$metal *= $shipfactor;
        $crystal *= $shipfactor;
					
        $defenseDerbisMetall *= $defensefactor;
        $defenseDerbisCrystal *= $defensefactor;

        return array($metal+$defenseDerbisMetall, $crystal+$defenseDerbisCrystal);
    }

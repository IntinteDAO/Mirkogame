...
class MissionCaseSpy extends MissionFunctions implements Mission
{
	...
	$def=0;
		...
		$lastActivity	= floor((time() - $targetPlanet['last_update']) / 60);
		...
		$surowce	= floor($targetPlanet["metal"]+$targetPlanet["crystal"]+$targetPlanet["deuterium"]);

		$sql	= 'SELECT * FROM %%FLEETS%% WHERE fleet_end_id = :planetId AND fleet_mission = 5 AND fleet_end_stay > :time AND fleet_mess = 2;';
		...
		if ($lastActivity < 15) {
			$online	= "(15 minut)";
		} elseif($lastActivity > 15 and $lastActivity < 60) {
			$online	= "(".$lastActivity." minut)";
		} else {
			$online	= '';
		}
			...
			foreach($elementIDs as $elementID)
			{
				...
				else 
				{
					$spyData[$classID][$elementID]	= $targetPlanet[$resource[$elementID]];
										if($elementID > 200 and $elementID < 600) $def+=$targetPlanet[$resource[$elementID]];
		...
		$template->assign_vars(array(
			...
			'surowce'			=> $surowce,
			'def'			=> $def,
			'mt'			=> floor((($surowce*0.55)/5000)+1),
			'dt'			=> floor((($surowce*0.55)/25000)+1),
			'isBattleSim'	=> ENABLE_SIMULATOR_LINK == true && isModuleAvailable(MODULE_SIMULATOR),
			'title'			=> sprintf($LNG['sys_mess_head'], $targetPlanet['name'], $targetUser['username'], $targetPlanet['galaxy'], $targetPlanet['system'], $targetPlanet['planet'], _date($LNG['php_tdformat'], $this->_fleet['fleet_end_time'], $targetUser['timezone'], $LNG)),
		));

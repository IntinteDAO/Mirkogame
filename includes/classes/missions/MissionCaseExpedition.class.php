...
class MissionCaseExpedition extends MissionFunctions implements Mission
{
	...
	function EndStayEvent()
	{
		...
		$GetEvent       = mt_rand(0, 100);
		
		
		
		
		if($GetEvent < 20) $GetEvent = 1;     /// surowce 20%
		if($GetEvent > 20 and $GetEvent < 40) $GetEvent = 3; //// flota 20%
		if($GetEvent > 40 and $GetEvent < 70) $GetEvent = 4; //// Kosmici 30%
		if($GetEvent > 70 and $GetEvent < 85) $GetEvent = 5; ///// Zabij flote  15%
		if($GetEvent > 85) $GetEvent = 6; ///// Spowolnij  15%
		...
		switch($GetEvent)
		{
			case 1:
				...
				$maxFactor		= floor($topPoints/2000);
				
$maxFactor = min($maxFactor,1500);

				$max_znajdz		= round(min($fleetCapacity,$maxFactor * $fleetPoints));
				$founded		= rand(min($fleetCapacity,5000),$max_znajdz);

$n = array(901=>"Metalu",902=>"Kryształu",903=>"Deuteru");


$Message .= ' Znalazłeś '.$founded.' '.$n[$resourceId];
		...
		$sql			= "SELECT * FROM uni1_planets WHERE id = :planetId;";

			$targetPlanet 	=  $db->selectSingle($sql, array(':planetId'	=> $this->_fleet['fleet_start_id']))	;

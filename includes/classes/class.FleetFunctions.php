...
class FleetFunctions 
{
	...
	private static function GetShipSpeed($Ship, $Player)
	{
		...
		$el = "speed";
				...
				if( $Ship == 209 and $Player['impulse_motor_tech'] >= 17)  {$techSpeed = 2;  $el = "speed2";}
								if( $Ship == 209 and $Player['hyperspace_motor_tech'] >= 15) {$techSpeed = 2;  $el = "speed3";}

			

				if( $Ship == 202 and $Player['impulse_motor_tech'] >= 5) {$techSpeed = 2;  $el = "speed2";}
				if( $Ship == 211 and $Player['hyperspace_motor_tech'] >= 8) {$techSpeed = 3;  $el = "speed2";}

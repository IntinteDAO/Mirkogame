...
class ShowOverviewPage extends AbstractGamePage
{
	...
	function show()
	{
		...
		$sql = "SELECT COUNT(onlinetime) AS onlineuserz FROM %%USERS%% WHERE onlinetime >= (UNIX_TIMESTAMP() - 15 * 60);";
        $onlineUsers = $db->select($sql);
        $onlineUsers = $onlineUsers[0]['onlineuserz'];

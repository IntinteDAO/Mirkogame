	...
	public function show()
	{
		...
		foreach($Elements as $Element)
		{
			...
			if(in_array($Element, $reslist['prod']))
			{
				...
				if($requireEnergy < 0) {
					$infoEnergy	= "Energia: <span style='color:red'>-".abs($requireEnergy)." </span>";
				} else {
					$infoEnergy	= "Energia: <span style='color:green'>+".abs($requireEnergy)." </span>";
				}
			}
...
if(isset($costResources[901]) and ($PLANET[$resource[901].'_perhour'] + $basicProduction[901]) > 0) { $wym['m']	= @floor($costResources[901]-$PLANET[$resource[901]])/max($PLANET[$resource[901].'_perhour'] + $basicProduction[901],1); }
if(isset($costResources[902]) and ($PLANET[$resource[902].'_perhour'] + $basicProduction[902]) > 0) { $wym['k']	= @floor($costResources[902]-$PLANET[$resource[902]])/max($PLANET[$resource[902].'_perhour'] + $basicProduction[902],1); }
if(isset($costResources[903]) and ($PLANET[$resource[901].'_perhour'] + $basicProduction[901]) > 0) { $wym['d']	= @floor($costResources[903]-$PLANET[$resource[903]])/max($PLANET[$resource[903].'_perhour'] + $basicProduction[903],1); }

$szer_w = 70;
	$czas = max($wym);
if($czas <= 0) $szer_w = 90;
		if($czas <= 0) $czas_t = "Rozbuduj do ".($levelToBuild+1)."!"; else $czas_t = "<span style='   font-size: 11px;    olor: #bbbbbb;    font-weight: 800;'>Dostępne o ".((date('d-m-y',time() + ($czas * 3600)) == date('d-m-y'))?date('G:i d-m-y ',time() + ($czas * 3600)):date('G:i d-m-y',time() + ($czas * 3600)))."</b></span>";

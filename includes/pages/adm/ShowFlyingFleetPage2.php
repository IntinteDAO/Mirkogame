...
function ShowFlyingFleetPage2()
{
	...
	if(	$all == 1){
		...
		$GLOBALS['DATABASE']->query("UPDATE ".FLEETS." SET `fleet_busy` = '0' WHERE fleet_end_time <= ".time()." AND `fleet_universe` = '".Universe::getEmulated()."';");
		

		
		$GLOBALS['DATABASE']->query("UPDATE ".FLEETS_EVENT." SET `lock` = NULL WHERE `lock` <> '' and time <= ".time());
		
		echo ' Unlock all flet evben';
	}
	...
	while($fleetRow = $GLOBALS['DATABASE']->fetch_array($fleetResult)) {
		...
		if($fleetRow['lock'] <> 'null')		$FleetList[]	= array(
	...
	$template->show('FlyingFleetPage2.tpl');
}
	...
	<div id="headerTable">
		{foreach $resourceTable as $resourceID => $resourceData}
			...
			{$resourceData.current = $resourceData.max + $resourceData.used}
				...
				używana {$resourceData.name}<br>
				wolna {$resourceData.name}
				</div>
				'>
			...
			<div class="pbox_frame" onclick="location.href = 'game.php?lista=1&page={if isset($smarty.get.page)}{$smarty.get.page}{else}overview{/if}&cp={$key}{if isset($smarty.get.menu)}&menu={$smarty.get.menu}{/if}{if isset($smarty.get.mode)}&mode={$smarty.get.mode}{/if}'" style="float: right;
    padding: 8px;cursor: pointer;
    background: #0e8a2" >{if {$lista_show}==1}Blokuj{else}Odblokuj{/if}</div>
				<!-- 	ressbar energy end
					darkmatter  start-->	
			{else}
...
<!-- planetselect planet activ start -->
	<div id="pbox" {if {$lista_show}==1}onmouseout="dorpit(this)" onmouseover="this.style.height='unset'{/if}">
		{if {$lista_show}==1}<div class="pbox2">
		...
		{foreach name=lol from=$PlanetSelect key=key item=Planet}
			...
			<div onclick="location.href = 'game.php?page={if isset($smarty.get.page)}{$smarty.get.page}{else}overview{/if}&cp={$key}{if isset($smarty.get.menu)}&menu={$smarty.get.menu}{/if}{if isset($smarty.get.mode)}&mode={$smarty.get.mode}{/if}'"  class="pbox" style="{if $current_pid == $key}opacity: 0.6;    background: #540202;{/if}{if ! $smarty.foreach.lol.last}border-bottom:1px solid #000;{/if} ">

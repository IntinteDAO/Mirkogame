...
<table class="table519">
	...
	{/if}
	<tr>
		<td style="white-space:nowrap;">{$LNG.ov_server_time}</td>
	...
	<tr>
		<td><a style="color:white">Staty:&nbsp;</a></td>

		<td>Graczy online:<a style="color:lime"> {$UsersOnline}</a></td>
		<td>
			Aktywnych flot:<a style="color:lime"> {$fleetsOnline}</a>
		</td>
	</tr>
	...
	<tr>
		<td> <Div id="fleettime_{$index}" class="fleets" data-fleet-end-time="{$fleet.returntime}" data-fleet-time="{$fleet.resttime}">{pretty_fly_time({$fleet.resttime})}</div><span style="color:#1aff1a;    font-size: 8px;">({$fleet.returntime|date_format:"%H:%M:%S %m/%d/%y"})</span></td>
